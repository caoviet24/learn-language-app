...
export default function TranslationSelectionHomeScreen() {
    ...
    const generateQuestionMutation = useMutation({
        mutationFn: async () => {
            const finalTopic = useCustomTopic ? customTopic.trim() : predefinedTopics.find(t => t.id === selectedPredefinedTopic)?.name;
            const selectedLanguageCode = languages.find(l => l.id === selectedLanguage)?.code;
            
            // Default language_out to English, or you can make it configurable
            const language_out = "en";
            const language_in = selectedLanguageCode || "en";
            
            return await questionService.generateQuestion({
                word: "",
                question_type: "translation",
                topic: finalTopic?.toString() || "",
                previous_question: "",
                language_in,
                language_out
            });
        ...
        onSuccess: (data) => {
            router.push({
                pathname: '/practice/translation',
                params: {
                    questionData: JSON.stringify(data),
                    questionType: 'translation',
                    language_in: languages.find(l => l.id === selectedLanguage)?.code,
                    language_out: 'en'
                }
            });
        },
        onError: (error) => {
            ...
            Alert.alert(t('common.error'), t('translation_selection.error_generating_question'));
        }
    });
    ...
    const handleStartPractice = () => {
        ...
        // Call the generate question API
        ...
        <ScrollView className="flex-1 bg-gray-50 pt-10">
            ...
            <View className="p-4 mt-6">
                ...
                <TouchableOpacity
                    ...
                    className={`mt-8 p-4 rounded-2xl items-center shadow-lg ${generateQuestionMutation.isPending
                        ? 'bg-gray-400'
                ...
                >
                    <Text className="text-white text-lg font-bold">
                        {generateQuestionMutation.isPending
                            ? t('translation_selection.generating_question')
